...
Key_Hover = keyboard_check_direct(ord('W'));
...
Key_Shoot = keyboard_check_direct(ord('K'));
if(!crouching)Key_Shoot_Release = keyboard_check_released(ord('K'));
...
Key_Teleport = keyboard_check_pressed(ord('H'));
...
if(Key_Teleport && !teleporting){
    if(scr_stamina_cost(155)){
        imageIndex = 0;
        teleporting = true;
    }
}
...
if (Key_Shoot_Release && !melee && !shooting){
    if(facing_Down || facing_Up || facing_Left || facing_Right){
        if(scr_stamina_cost(70)){
            imageIndex = 0;
            shooting = true;
            Charge = 0;
        }
...
if(crouching){
    sprite_index = spr_vairl_collision_crouching;
} else {
    sprite_index = spr_vairl_collision;
}
if(teleported){//teleport has priority over all others
    vsp = 0;
    hsp = 0;
}
if(tpForce){
